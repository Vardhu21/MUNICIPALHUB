import { computeClock, slaRow, TIER_LABEL, type Priority, type Tier } from "@/lib/sla";
...
export function SlaBar({
  ...
  const clock = computeClock(createdAt, priority, offsetHours);
    ...
    <div className="space-y-1.5">
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 text-xs">
        <span className="min-w-0 truncate text-muted-foreground">
          {slaRow(priority).hours}h SLA · {TIER_LABEL[tier]?.[lang]}
        </span>
